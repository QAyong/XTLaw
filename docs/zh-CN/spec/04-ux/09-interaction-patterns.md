...
- 在第一个助手或工具事件之前，活动回合显示一个紧凑的
  ...
  仍在运行的 Subagent 的粗粒度动作）。该行与它所解释的思考/工具行共用助手
  栏的左缘与文字轨，因此不会比它所解释的工作缩进得更深。一旦出现具体的思考/工具/
  回答反馈或内联权限卡，该行就被替换。
...
展开侧边栏默认为 275px，可通过右边缘手柄在 240px–520px 之间调整。折叠/展开只
改变列是否存在；首选宽度会独立于 48px 图标导轨继续持久化。
