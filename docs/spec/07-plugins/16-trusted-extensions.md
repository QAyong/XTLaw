...
| Class | Members |
|---|---|
| Supported | `registerTool`, `registerCommand`, `registerAgent`, `registerProvider` (plugin-owned compatibility alias; same shape as `registerAgent`), `unregisterAgent`, `unregisterProvider`, `on(...)` for every event in §6, `exec`, `getActiveTools`, `getAllTools`, `setActiveTools`, `getCommands`, `setModel` (configured models and plugin agents; idle-only; persists the current session binding), `getThinkingLevel`, `getThinkingLevels`, `setThinkingLevel` (ADR 0257: accepts `{ persist }`; `persist: true` updates the session baseline through `session.configure`, `persist: false` is turn-scoped and reverts at turn end), `setSessionName`, `getSessionName`, `sendUserMessage` (Host-owned queue, D386), `getFlag` |
