...
- The right toolbar exposes one combined model × reasoning trigger immediately
  ...
  trigger shows the model's provider icon — the Bot glyph when the provider and
  model name match no known vendor — the current model, and reasoning level; `off` omits
