# 11. asktool问题卡

> **翻译说明：** 本页是与 [英文源规格](/spec/04-ux/11-asktool-question-card) 一一对应的机器辅助翻译。代码、协议字段和标识符保持原文；如翻译与英文源事实有歧义，以英文版本为准。


Asktool 卡是内联输入框批准表面，而不是许可
对话框。它与 Plan 和 Goal 批准卡安装在同一停靠区域，
紧邻输入框输入的上方，因此暂停的问题仍然可用
主动决策点，而不是进入成绩单历史。

它使用现有的消息宽度、边框、背景和按钮标记，以便暂停的问题在
视觉上仍然是对话的一部分。问题文本比聊天正文大半档
（`--text-base-plus`，15 px），因此它读起来是卡片的主要焦点，
同时仍贴近周围成绩单的字号。

标题标识提示并显示进度。可点击的小指示器
编码已应答、未应答、跳过和当前状态，而不与
问题文本。选择控件使用单选语义来进行单选和
多选的复选框语义。每个问题都包含一个可见的自定义
输入选择。操作行包含 Skip 和 Next/Submit，其中 Decline 全部为
标题中安静的次要动作。

该卡没有倒计时或到期副本。在窄屏幕上，选项仍然存在
全角和动作可以共享行；问题文本和自定义输入可能
自然包裹，无需剪裁。

## 收起

标题栏在「全部拒绝」旁边提供收起按钮。收起后只保留标题栏：标题、进度、
「全部拒绝」以及该按钮；问题正文——状态指示点、问题文本、选项、自定义输入
以及「跳过 / 下一题（提交）」一行——全部隐藏。这样暂停中的提问不会遮挡用户
仍在阅读的助手输出，剩下的标题栏以更小的上下内边距呈现为一行安静的信息。

该按钮是 24 px 的图标按钮，使用共享的箭头图标，文案取自
`askTool.collapse` / `askTool.expand`，通过 `aria-expanded` 汇报状态，并用
`aria-controls` 指向被隐藏的正文。卡片默认展开，收起状态属于卡片本地状态、
不做持久化——新的提问总是以展开状态出现。拒绝、跳过与当前题号不受收起影响。

## 版式层级

该卡使用清晰的四级字号阶梯，以避免过多相近尺寸带来的视觉噪音：

| 元素 | 标记 | 尺寸 | 字重 | 备注 |
|----------------------|--------------------|---------|------------|-----------------------------|
| 卡片标题 | `--text-xs` | 11 px | medium | 全大写，宽字距 |
| 问题编号 | `--text-xs` | 11 px | medium | 宽字距，眉标角色 |
| 进度 | `--text-2xs` | 10.5 px | — | 淡色，最不显眼 |
| 问题文本 | `--text-base-plus` | 15 px | medium | 主要焦点 |
| 选项文本 | `--text-sm-plus` | 12.5 px | — | 比问题低一档 |
| 选项标记 | `--text-xs-plus` | 11.5 px | — | 与选项文本成比例 |
| 自定义输入文本 | `--text-sm-plus` | 12.5 px | — | 与选项文本一致 |
| 拒绝按钮 | `--text-xs` | 11 px | — | 安静的次要动作 |
