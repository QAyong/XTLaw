...
输入法才能唤出菜单。「、」与 `/` 是同一条触发规则：两者都可以在草稿的第 1 个字符、
且光标仍位于第一个不含空白的 token 内时打开命令模式，随后按上文相同的过滤、键盘
导航与插入行为使用普通斜杠菜单。该位置的「、」同时会被规范化为 `/`，因此草稿、
转录 chip 与发送路径只会看到普通的斜杠调用（D447）。

只有第 1 个字符是触发符。出现在草稿其他位置的「、」属于普通标点，始终不会被改动；
该别名对 `@` 文件菜单没有影响。
