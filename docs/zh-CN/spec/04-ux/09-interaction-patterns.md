...
以上 450px 下限描述默认聊天布局。工作布局把工作面板放在中间、聊天放在右侧，
分隔条移到工作面板右边缘并调整右侧聊天目标宽度：目标默认及最低均为 450px，
中间工作区至少 300px，空间不足时先收起侧边栏。右上角固定按钮控制右侧聊天，
`Cmd/Ctrl + J` 仍控制工作面板。布局与目标宽度持久化，临时收起聊天不持久化。

- 默认聊天布局的 10px 左边缘分隔符锚定到按下位置和起始宽度，然后跟随指针 delta 而不跳转。向左拖动会持续增长面板，直到共享预算耗尽；MainChat 到达 450px 下限时展开的左栏立即收起。向右拖动把空间还给 MainChat。工作布局的分隔条位于面板右边缘，拖动向右会加宽中间工作区，向左会加宽右侧聊天。
