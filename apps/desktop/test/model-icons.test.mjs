...
test("all model icon symbols exist in the packaged relative sprite", () => {
  ...
  const symbols = [...source.matchAll(/symbol:\s*"([^"]+)"/g)].map((match) => match[1]);
...
test("the committed sprite is up to date with its vendored sources", () => {
  const generator = resolve(root, "../../scripts/provider-icons/build.mjs");
  const output = execFileSync(process.execPath, [generator, "--check"], {
    encoding: "utf8",
  });
  assert.match(output, /sprite is up to date/);
});
