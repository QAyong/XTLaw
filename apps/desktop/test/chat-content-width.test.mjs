...
test("chat width handles are invisible at rest and show a quiet capsule on hover", () => {
  ...
  assert.match(styles, /:root\[data-theme="light"\] \.chat-width-handle::before/);
  assert.match(
    styles,
...
test("dragging the width also scales the per-message prose width", () => {
  assert.match(handleSource, /setProperty\("--chat-content-max-width",\s*px\)/);
  assert.match(handleSource, /setProperty\("--chat-composer-max-width",\s*px\)/);
  assert.match(handleSource, /setProperty\("--chat-prose-max-width",\s*px\)/);
  assert.match(styles, /--chat-prose-max-width:\s*var\(--chat-content-max-width\)/);
});
