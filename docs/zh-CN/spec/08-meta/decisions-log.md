...
| D447 | 顿号就是首个斜杠触发符 | **修订 D405 / ADR 0231：「、」（U+3001）与 `/` 是同一条命令模式触发规则。`detectTrigger` 接受两者作为草稿的第 1 个字符，只要光标仍位于第一个不含空白的 token 内即可打开菜单，因此「、」与输入的 `/` 一样能唤出菜单，不再依赖渲染器是否已经把该字符改写进编辑区。首个「、」仍会在输入时规范化为 `/`，该规范化改为遵循与 `/` 相同的定位规则，不再限于此前为空的草稿。插入、过滤、发送、转录 chip 与 `@` 文件菜单均无变化。仅共享语法与渲染器改动；不改 IPC、存储或补全数据源。见 ADR 0231 修订段与 E2E-255。** | 该别名此前只实现为一次输入改写，并以草稿此前为空为前提，覆盖的状态比 `/` 本身更窄：已经含有文字的草稿，以及改写没有落到编辑区的输入路径，都会让「、」保持普通标点且菜单不打开。与 `/` 共用语法里的触发规则可以去掉这个依赖，而插入与发送路径所见的内容不变。 |
...
| D434 | 恢复展开侧边栏的用户调整宽度 | **修订 ADR 0238 / D408：恢复渲染器拥有的侧边栏调整手柄，范围为 240px–520px，默认为 275px。指针预览保持锚定且可安全取消；释放时持久化首选宽度，键盘调整立即提交，折叠时保留与 48px 图标导轨分离的宽度偏好。MainChat 的 450px 下限、工作面板预算、自动让位和固定原生窗口边界不变。见 ADR 0267 与 E2E-168。** | 固定宽度修订禁用了仍受支持的侧边栏操作，也让用户更难查看较长的项目/会话标题。 |
| D409 | 宿主拥有的会话协作消息 | **修订 ADR 0237 / ADR 0165 / ADR 0213：Rust host-core 拥有以消息 id 和真实源/目标 Session ID 为键的持久会话协作 ledger。插件驱动的 `spawn`、`send`、`status`、`result` 和 `cancel` 使用已审查的 desktop-control 网关；发送者绑定当前插件 Agent 工具调用，目标回合保留原有配置，每条投递由实际持久回合认领。完成回调持久化且最多一次，并引用已结算回合。只有当前由 Host 账本解析的完成提示允许静默结束而不触发空响应恢复；后续运行及接受用户 steering 后恢复普通响应契约。来源信息随转录行持久化，不能在重生成中伪造、剥离或编辑。增量架构 v16 迁移在重启后保留排队工作但不无人值守重放，执行权限上限和有界自主跳数，同时保持既有 Task 系列不变。见 ADR 0239 与 E2E-PLUGIN-session-orchestrator-real-workers。** | 插件之前的创建/提示轮询路径既无法推断持久回合结果，也无法安全确认双向发送者身份。宿主拥有的 ledger 让投递、来源、回调、取消和重启行为可审计，同时不恢复已撤回的 A2A 协议。 |
...
## 2026-09-01 —— 展开侧边栏宽度可由用户调整（D280；由 D408 修订并由 D434 恢复）
...
- D408 曾将当前 shell 中的展开侧边栏固定为 275px；D434 恢复了显式用户调整
  宽度的契约，同时保留 D408 的 MainChat 优先级和自动让位规则。
...
## 2026-09-16 —— 恢复展开侧边栏的用户调整宽度（D434）

- 恢复渲染器拥有的右边缘侧边栏分隔器。展开宽度默认为 275px，按 240px–520px
  四舍五入并限制范围，从指针按下位置锚定预览，指针释放时持久化，键盘调整立即提交；
  Escape、取消、捕获丢失和卸载会恢复指针手势的起始宽度。
- 折叠/展开只改变侧边栏列是否存在；首选展开宽度与 48px 图标导轨分离，并在重启后恢复。
- D408 的 450px MainChat 下限、动态工作面板预算、自动让位和固定原生窗口边界保持不变。
  见 ADR 0267 与 E2E-168。
...
- 侧边栏会话项与项目菜单现在第一次点击只武装删除，第二次点击才移除行。共享的
  `useArmedDelete()` hook 负责 3200ms 超时，并保持会话与项目的武装状态相互隔离。
- 被武装的菜单项带 `data-armed="true"`、危险色表面和确认标签；点击外部、按 Escape
  或超时都会解除武装而不删除。仍有运行中任务的项目继续使用现有确认对话框和停止后删除流程。
- 仅渲染层：不改变协议、存储、host、权限或迁移，磁盘上的文件夹仍不会被触碰。见
  `04-ux/09-interaction-patterns.md`、D421、D431 与 E2E-SESSION-two-click-delete-arms-first。

## 2026-09-17 —— 按请求实际会走的线路判定公网地址（D436）

- 技能市场 HTTPS 检查现在按请求实际会走的线路判定每个重定向跳。`proxied` 线路只放行解析器产物的 benchmark/fake-IP 类别；`direct` 或未知线路继续严格拒绝。
- 拒绝结果和市场 failure details 会带上判定线路；每一跳独立判定，MCP 市场原有的 Node HTTPS 守卫不变。
- 见 ADR 0272、`05-security/01-security.md` §4.1、`03-runtime/09-logging-and-observability.md` 与 E2E-SKILL-MARKET-NET-BOUNDARY。

## 2026-09-19 —— 顿号就是首个斜杠触发符（D447）

- `detectTrigger` 现在把「、」（U+3001）与 `/` 视为同一条命令模式触发规则：两者都可以在
  草稿的第 1 个字符、且光标仍位于第一个不含空白的 token 内时打开菜单；唤出菜单不再依赖
  输入改写是否已经把该字符替换进编辑区。
- 首个「、」仍会在输入时规范化为 `/`，条件改为与 `/` 相同的定位规则，不再限于此前为空的
  草稿。插入、过滤、发送、转录 chip 与 `@` 文件菜单均无变化。
- 决策 D447 修订 D405 / ADR 0231。见 `04-ux/04-builtin-commands.md` §9 与 E2E-255。
