...
每个 binding 还记录 `contextWindow` 的来源（`contextWindowSource`）：

- `catalog`：该值来自 models.dev 快照，后续目录修正 `limit.context` 时会更新；
  被修正上限的模型无需删除重建。
- `user`：该值来自 Advanced 中的手改或预设档位，任何目录修正都不会覆盖，
  包括手改的 `128,000`。

在来源标记出现前保存的 binding 没有来源信息，仍按确定的历史规则解析：只有恰好等于
128,000 的通用种子跟随已发布目录值，其余已存值保持不变。标记是可选的，旧版本配置
仍可读取，降级版本会忽略它。
