...
describe("chat content width", () => {
  ...
  it("compresses to the pane when the sidebar squeezes past the preference", () => {
    expect(clampChatContentMaxWidth(1100, 700)).toBe(
      700 - 2 * CHAT_CONTENT_WIDTH_GUTTER,
    );
  ...
  it("lets a tiny pane drop below the drag floor", () => {
    expect(clampChatContentMaxWidth(760, 500)).toBe(
      500 - 2 * CHAT_CONTENT_WIDTH_GUTTER,
    );
  });
