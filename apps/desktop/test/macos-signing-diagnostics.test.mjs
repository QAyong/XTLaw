...
function appFixturePath(release) {
  return join(release, "mac-arm64", "XTLaw.app");
}
 ...
 *   Contents/MacOS/XTLaw                            mach-o, executable
...
async function writeBundleFixture(release) {
  ...
  await writeMachO(join(app, "Contents", "MacOS", "XTLaw"), 4100);
  await writeMachO(
...
test("inventory counts the signing payload of a release directory", async (t) => {
  ...
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
  assert.ok(
    result.stdout.includes(`==> XTLaw.app inventory: ${app}`),
    result.stdout,
  );
  ...
  assert.match(
    result.stdout,
    /^slowest-likely: Contents\/MacOS\/XTLaw \(4100 bytes\), Contents\/Resources\/bin\/pi-desktop-host-core \(2052 bytes\)$/m,
  );
  ...
  assert.equal(json.slowestLikely[0].path, "Contents/MacOS/XTLaw");
...
test("inventory counts binary-looking resources as signing candidates", async (t) => {
  ...
  // is 1 + 1 + 1 = 3.
  await writeMachO(join(app, "Contents", "MacOS", "XTLaw"), 512);
...
test("inventory requires exactly one app bundle under the given path", async (t) => {
  ...
  await writeFileWithParents(
    join(twoApps, "mac-arm64", "XTLaw.app", "Contents", "Info.plist"),
    "<plist/>",
  );
  await writeFileWithParents(
    join(twoApps, "mac-x64", "XTLaw.app", "Contents", "Info.plist"),
    "<plist/>",
  );
  ...
  const fileResult = runInventory([join(root, "two-release", "mac-arm64", "XTLaw.app", "Contents", "Info.plist")]);
...
test("inventory survives a symlink cycle without hanging or double counting", async (t) => {
  ...
  const release = join(root, "release");
  const app = appFixturePath(release);
  await writeFileWithParents(join(app, "Contents", "MacOS", "XTLaw"), "not-mach-o");
