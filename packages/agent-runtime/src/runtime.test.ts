...
describe("DesktopAgentRuntime deferred tool catalog", () => {
  it("scopes read_session and distinguishes explicit from promoted steering", async () => {
    const runtime = createRuntime();
    const agent = (runtime as any).agent;
    ...
    runtime.steer(
      { text: "Promoted queued input." },
      "turn-1",
      {
        id: "steer-promoted",
        role: "user",
        content: "Promoted queued input.",
    ...
    expect((runtime as any).turnSessionReferenceIds).toEqual(new Set(["past-session"]));
    runtime.steer(
      { text: "Continue without that reference.", sessionReferences: [] },
      "turn-1",
      {
        id: "steer-1",
        role: "user",
