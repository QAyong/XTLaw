...
const [
  ...
  read("../src/features/chat/composer/hooks/useComposerAttachments.ts"),
...
test("send clears the composer before the round trip and restores a rejected draft (D287)", () => {
  ...
  assert.match(composerAttachments, /snapshotSessionReferences/);
  assert.match(composerAttachments, /sessionReferences: previousSessionReferences/);
});
...
test("the user row is inserted before the host round trip and echoed under the same id (D288)", () => {
  ...
  assert.match(editUserMessage, /sessionReferences/);
  const retryLastPrompt = transcriptSlice.slice(
    transcriptSlice.indexOf("retryLastPrompt: async"),
  );
  assert.match(retryLastPrompt, /sessionReferences/);
