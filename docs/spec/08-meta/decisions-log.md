...
| D434 | Restore user-resizable expanded sidebar width | **Amend ADR 0238 / D408: restore the existing renderer-owned sidebar resize handle with a 200px–520px clamp and 275px default. Pointer preview remains anchored and cancellation-safe; release persists the preferred width, keyboard changes commit immediately, and collapse keeps the preference separate from the 48px icon rail. MainChat's 450px floor, work-panel budget, user-controlled sidebar state, and fixed native-window bounds remain unchanged. See ADR 0267 and E2E-168.** | The fixed-width amendment disabled a still-supported sidebar affordance and made long project/session labels harder to inspect. |
...
| D435 | Development-only Windows shell identity | **Amend D141 / D121: only a packaged run registers the canonical `com.pi-desktop.app` AppUserModelID before readiness; an unpackaged run registers the development-only `com.pi-desktop.app.dev`, the literal already used as the macOS development bundle identifier. The Windows notification platform creates a Start Menu shortcut for the AppUserModelID of whichever process shows a toast, named and iconed after that process's own executable, and the shell resolves a window's AppUserModelID through that shortcut, so a development host registering the shipped ID leaves an Electron-branded shortcut that owns the installed app's notifications, taskbar group, taskbar name, and taskbar icon. Windows shell surfaces in development are intentionally development-only; packaged surfaces keep D141 unchanged. No appId, executable name, NSIS shortcut identity, protocol, storage, permission, or plugin contract changes.** | Shell identity resolution is Windows shortcut lookup, so a development host must never register the identity the installed app answers to |
...
- The expanded sidebar owns a persisted preferred width with a `200..520px`
...
- D408 temporarily superseded the live shell behavior by fixing the expanded
  ...
  retaining D408's MainChat priority and work-panel budget.
...
- The in-flow shell treats MainChat as the first-priority column: a hard 450px
  ...
  sidebar`, no fixed maximum), and a user-controlled expanded sidebar that
  remains unchanged when the budget is exhausted. A manually collapsed sidebar
  still counts toward the budget during its exit animation.
- A manual sidebar reopen spends work-panel width first and otherwise targets
  460px. Closing the panel preserves the user's sidebar state.
...
- Restore the existing renderer-owned right-edge sidebar separator. The
  expanded width defaults to 275px, rounds and clamps to 200px–520px, previews
...
- D408's 450px MainChat floor, live work-panel budget, user-controlled sidebar
  state, and fixed native-window bounds remain unchanged. See ADR 0267 and
  E2E-168.
...
## 2026-09-17 — Windows development stops claiming the shipped shell identity (D435)

- A Windows development run registers the development-only
  `com.pi-desktop.app.dev` AppUserModelID; only a packaged run registers the
  canonical `com.pi-desktop.app`. This amends D141 and leaves D121's
  development-host contract intact: Windows/Linux development still launches the
  stock electron-vite executable.
- Why: the Windows notification platform creates a Start Menu shortcut for the
  AppUserModelID of whichever process shows a toast, named and iconed after that
  process's own executable, and the shell resolves a window's AppUserModelID
  through that shortcut. A development run on the shipped ID therefore left an
  `Electron`-branded shortcut that carried `com.pi-desktop.app`; the installed
  app then appeared in the taskbar as `Electron` with Electron's default icon
  even though its executable, embedded icon, window icon, and NSIS shortcut were
  all correct.
- Windows shell surfaces in development are now intentionally development-only,
  matching the macOS development host bundle; `PI-Desktop` branding comes from
  the installed package.
- No appId, executable name, NSIS shortcut identity, protocol, storage,
  permission, or plugin contract changes. See ADR 0268,
  `03-runtime/01-ipc-protocol.md`, `04-ux/07-ui-design-system.md`,
...
- Sidebar session items and project menus now arm destructive deletion on the
  first click and remove the row only on the second click. The shared
  `useArmedDelete()` hook owns the 3200ms expiry and keeps session and project
  arms separate.
- An armed item carries `data-armed="true"`, a danger surface, and the
  confirmation label. Outside press, Escape, or expiry clears the arm without
  removing anything. Running projects still use the existing confirmation
  dialog and stop-and-delete flow.
- Renderer only: no protocol, storage, host, permission, or migration change;
  the folder on disk remains untouched. See `04-ux/09-interaction-patterns.md`,
  D421, D431, and E2E-SESSION-two-click-delete-arms-first.

## 2026-09-17 — Judge a public-network address on the route the request will dial (D436)

- Skill-market HTTPS checks now classify each redirect hop using the route that the request will actually dial. A proxied route accepts only the resolver-artifact benchmark/fake-IP class; direct or unknown routes remain strict.
- Refusals and market failure details include the judged route. Redirect hops are judged independently; MCP market’s pinned Node HTTPS guard is unchanged.
- See ADR 0272, `05-security/01-security.md` §4.1, `03-runtime/09-logging-and-observability.md`, and E2E-SKILL-MARKET-NET-BOUNDARY.

## 2026-09-18 — Let users resize the centered conversation band (D439)

- MainChat now gives the transcript, empty-home stack, and Composer one
  persisted preferred width, defaulting to 760px. Both centered edge handles
  update the same width, and the live band compresses to the available pane
  without overwriting the preference.
- Handles stay quiet at rest, expose keyboard separator semantics, support
  pointer drag, Arrow/Home/End, Escape cancellation, and double-click reset.
  Message prose and process surfaces consume the same width variable so a
  widened band does not leave the AI output frozen at the old 720px cap.
- Renderer and existing settings persistence only; no host schema, IPC, or
