...
- 列表变为以 2–6px 间距堆叠的色块行（设置行、快捷键表、能力页、服务与模型行、
  ...
  强调色焦点环。Markdown 去掉代码块、表格、引用块与标题的装饰线（斑马行、色块、
  留白）；作者明确写出的水平分隔线仍显示为适配主题的 1px 细线，因为它属于正文内容，
  而非界面装饰。子智能体树在色块组上用 2px 着色条作连接线，节点抬起。
...
## 2026-09-23 —— 裸 URL 的边界就是 URL 本体的边界（D621）

- 聊天链接扫描在 URL 本体无法直接承载的字符处结束裸 URL（RFC 3986 加 `%`），
  而不是等到下一个空白字符。中文正文紧跟在粘贴的 URL 之后、中间没有分隔符，因此按空白
  界定的扫描会把整句话——包括其后由输入框序列化的 `@file` 引用——当成一个 URL，
  在气泡里整段渲染成超链接。
- URL 中成对的圆括号、正文包裹网址时多出的右括号、紧跟右括号的句末标点，都保持
  `04-ux/08-component-spec.md` §5 记录的既有行为；ASCII 与百分号编码的路径保持完整，
  未编码的非 ASCII 路径则在第一个这样的字符处结束链接。
- 仅渲染层：未改动宿主协议、IPC、存储与权限。回归覆盖在 `chat-links.test.mjs`。

## 2026-09-23 —— 引用出现在句子里也是芯片（D622）

- 聊天链接解析把 Windows 分隔符当作分隔符：`@C:\…\合同.docx` 与 `C:/…/合同.docx`
  一样是路径；绝对路径的 `@` 引用保留输入框自己的写法，因此气泡里的芯片指向的正是那份被
  粘贴的附件。文档、压缩包、媒体与截图进入已知扩展名列表，所以附件里写 `合同.docx`
  会像 `docs/合同.docx` 一样生成芯片，而不是留成纯文本。
- 扩展名必须是一段短 ASCII 字符；被中文粘连的引用只把引用本身做成芯片
  （`看 @docs/a.md，然后呢`），而不是造出一个永远打不开的 `a.md，然后呢`。
  名字里确实带这些字符的情况（`报告（终稿）.md`）仍按原样解析。
- 仅渲染层：未改动宿主协议、IPC、存储与权限。回归覆盖在 `chat-links.test.mjs`，
  芯片规则记录在 `04-ux/08-component-spec.md` §5。

- 被截短的引用按普通候选校验；助手 markdown 里的绝对文件引用以百分号编码传递，
  这样 URL 转换与净化 schema 都会保留该值，锚点在打开前解码（包含校验仍由
  `fs/resolveRef` 负责）。
